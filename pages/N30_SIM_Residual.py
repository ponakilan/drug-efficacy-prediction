...
# Display DataFrames
st.subheader("N30 Dataset")


#st.subheader("Multi Linear Regression RMSE Results:")
#st.table(n30mlr)




#st.subheader("Support Vector Regression RMSE Results:")
#st.table(n30svr)
...
if st.button("Predict with MLR+SVM"):
    output_df = predict_svr_residual(X,input_df,X_scaled,y,label)
    #print(output_df)
    st.success(f"MLR Prediction: success")
    st.table(output_df)

if st.button("Predict with MLR+XGBoost"):
    output_df = predict_residual_xgboost(scaler,X,input_df,X_scaled,y,label)
    #print(output_df)
    st.success(f"XGBoost Prediction: success")
    st.table(output_df)
